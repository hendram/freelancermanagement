...
export default function UpdateResume({ goBackU, goToUpdatePage }) {
...
const updateResume = (resumeData) => {
  goToUpdatePage(resumeData); // you'll handle routing in parent component
};
...
<button   className="update-btn"   onClick={() => updateResume(resumes[index])} >
  Update Resume
</button>
            <button className="delete-btn"   onClick={() => handleDelete(resumes[index].id)} >
             Delete
            </button>
          </div>
        </div>
